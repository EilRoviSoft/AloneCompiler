...
namespace alone::isa {
	template<typename T>
	size_t inst_b23(const context_t& ctx, const lib::arguments_metadata_t& args_metadata, const std::function<T(const T&, const T&)>& pred) {
	...
	template<typename T>
	size_t inst_u12(const context_t& ctx, const lib::arguments_metadata_t& args_metadata, const std::function<T(const T&)>& pred) {
		size_t offset = lib::inst_code_size;
		bool is_reduced = false;
		T* a0;
		T a1;
	...
	template<typename T>
	size_t inst_cmp(const context_t& ctx, const lib::arguments_metadata_t& args_metadata) {
		size_t offset = lib::inst_code_size;
		T a0, a1;
